...
# --- 4. (NEW) EFFICIENT DB OBJECT LOADER ---

def get_db_objects():
    """
    Creates the engine and fetches all schemas ONCE.
    This is for efficiency, so we don't do it in a loop.
    Returns (engine, all_schemas)
    """
    logging.info("Initializing Databricks connection and fetching all schemas...")
    engine = get_databricks_engine()
    
    if engine is None:
        logging.critical("Failed to create Databricks engine.")
        return None, None
        
    all_schemas = get_all_table_schemas(engine)
    
    if not all_schemas:
         logging.warning("Engine created, but no table schemas were found.")
         # We can still proceed, but recommendations might be empty
         return engine, {}

    logging.info(f"Successfully loaded engine and {len(all_schemas)} table schemas.")
    return engine, all_schemas
